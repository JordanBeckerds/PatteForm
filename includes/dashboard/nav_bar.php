...
$nav_pages = [
    ...
    'users'             => ['icon' => 'user',      'label' => 'Utilisateurs'],
];
...
$hmt = htmlspecialchars($homepage_main_color_text, ENT_QUOTES, 'UTF-8');
?>

<aside style="background-color:<?= $cp ?>" class="w-64 min-h-screen shadow-md">
  <div style="color:<?= $hmt ?>" class="p-6 text-center font-bold text-4xl">Dashboard</div>
  <nav class="mt-6">
    <?php foreach ($nav_pages as $key => $data): ?>
      <a href="dashboard.php?page=<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>"
         class="flex items-center p-4 mb-2 rounded transition-colors"
         style="<?= $current_page === $key ? 'background-color:' . $cs . ';' : '' ?>">
        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" stroke-width="2"
             style="color:<?= $ct ?>">
          <use href="#<?= htmlspecialchars($data['icon'], ENT_QUOTES, 'UTF-8') ?>"/>
        </svg>
        <?= $data['label'] ?>
      </a>
    <?php endforeach; ?>
    <a href="logout.php" class="flex items-center p-4 mb-2 rounded transition-colors hover:bg-red-100 text-red-600">
      D&#233;connexion
    </a>
  </nav>
</aside>
