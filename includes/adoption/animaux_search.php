...
<div class="flex flex-col lg:flex-row p-4 gap-6">
  <!-- FILTER SIDEBAR -->
  <form method="GET" class="w-full lg:w-1/4 bg-gray-50 rounded-xl p-4 shadow-md space-y-4" autocomplete="off">
    <h2 class="text-lg font-semibold">Filtres</h2>

    <input type="text" name="search" placeholder="Rechercher..." value="<?= htmlspecialchars($search) ?>" class="w-full p-2 border rounded" />
    ...
    <div class="relative">
      <input type="text" name="race" id="raceInput"
        ...
        value="<?= htmlspecialchars($raceFilter) ?>"
    ...
    <div class="flex gap-2">
      <input type="number" name="min_age" placeholder="Âge min" value="<?= htmlspecialchars($min_age) ?>" class="w-1/2 p-2 border rounded" />
      <input type="number" name="max_age" placeholder="Âge max" value="<?= htmlspecialchars($max_age) ?>" class="w-1/2 p-2 border rounded" />
    </div>

    <label class="flex items-center space-x-2">
      <input type="checkbox" name="enfant" <?= $enfant === 1 ? 'checked' : '' ?> />
      <span>Ami avec enfants</span>
    </label>
    <label class="flex items-center space-x-2">
      <input type="checkbox" name="chat" <?= $chat === 1 ? 'checked' : '' ?> />
      <span>Ami avec chats</span>
    </label>
    <label class="flex items-center space-x-2">
      <input type="checkbox" name="chien" <?= $chien === 1 ? 'checked' : '' ?> />
    ...
    <label class="flex items-center space-x-2">
      <input type="checkbox" name="sos" <?= $sos === 1 ? 'checked' : '' ?> />
      <span>SOS uniquement</span>
    </label>

    <label class="flex items-center space-x-2">
      <input type="checkbox" name="favorite" <?= $favoriteFilter === 1 ? 'checked' : '' ?> />
      <span>Favoris uniquement</span>
    </label>

    <button type="submit" class="bg-blue-600 text-white w-full p-2 rounded hover:bg-blue-700">Rechercher</button>
  </form>
  ...
  <div class="w-full lg:w-3/4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
    <?php foreach ($animaux as $animal): ?>
      ...
      ?>
      <a href="adoption.php?id=<?= $id ?>" class="block bg-white rounded-lg shadow-lg overflow-hidden relative flex flex-col h-[42.7vh] hover:shadow-xl transition-shadow duration-300">
        <form method="POST" class="absolute top-2 left-2 z-10" onclick="event.stopPropagation();">
          ...
          <button type="submit" class="text-4xl <?= $isFavorite ? 'text-red-500' : 'text-gray-400' ?>" aria-label="Toggle favorite">
            <?= $isFavorite ? '❤️' : '🤍' ?>
          </button>
        </form>
        <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($animal['nom']) ?>" class="w-full h-[65%] object-cover">
        <div class="p-4 flex flex-col flex-grow">
          <h3 class="text-xl font-semibold"><?= htmlspecialchars($animal['nom']) ?></h3>
          <p class="text-gray-600"><?= htmlspecialchars($animal['race']) ?> - <?= htmlspecialchars($animal['age']) ?> an(s)</p>
          <p class="text-sm mt-2 overflow-hidden line-clamp-3"><?= htmlspecialchars($animal['description']) ?></p>
        </div>
      </a>
    <?php endforeach; ?>
  </div>
