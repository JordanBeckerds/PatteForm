...
<main class="container mx-auto px-6 py-16 flex flex-col md:flex-row gap-16 min-h-[100vh]">
  ...
  <aside class="w-full md:w-1/4 bg-white rounded-2xl shadow-lg p-8 md:sticky md:top-56 h-fit">
    ...
    <input
      ...
      class="w-full px-6 py-4 border border-gray-300 rounded-xl focus:outline-none focus:ring-4 focus:ring-[#F97316] focus:border-[#F97316] text-lg"
      autocomplete="off"
    />
  </aside>
    ...
    <?php else: ?>
      <?php foreach ($actualites as $act): ?>
        <a href="actualites.php?id=<?= $act['id'] ?>" class="block">
          <article class="actualite-card bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row gap-8 md:gap-14 p-8 md:p-14 transition hover:shadow-[0_15px_40px_rgba(0,0,0,0.1)] min-h-[300px] hover:cursor-pointer">
            ...
            <div class="flex flex-col justify-between w-full">
              ...
              <time class="text-sm md:text-lg text-gray-400"><?= date('d/m/Y', strtotime($act['date_publication'])) ?></time>
            </div>
          </article>
        </a>
      <?php endforeach; ?>
